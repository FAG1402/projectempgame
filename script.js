...
window.addEventListener('load', sesuaikanUkuranLayar);
...
function bukaLayar(idLayarTujuan) {
    ...
    if (idLayarTujuan === 'screen-home') {
        ...
        if (bgmHome && bgmHome.paused) {
            ...
            bgmHome.play().catch(e => console.log("Menunggu interaksi pengguna"));
        }
        const btnContinue = document.getElementById('btn-main-continue');
        if (btnContinue) {
            if (localStorage.getItem('project_emp_save_state')) {
                btnContinue.classList.remove('hidden-ui'); // Tampilkan jika ada save
            } else {
                btnContinue.classList.add('hidden-ui');    // Sembunyikan jika tidak ada save
            }
        }
    }

    // ==========================================
    // TAMBAHAN: BERSIHKAN EFEK MATI LAMPU SAAT KELUAR GAME / ENDING
    // ==========================================
    if (['screen-home', 'screen-ending', 'screen-gameover'].includes(idLayarTujuan)) {
        isBlackout = false;
        isSenterOn = false;
        if (typeof intervalListrik !== 'undefined') clearInterval(intervalListrik);
        
        const overlayMatilampu = document.getElementById('blackout-overlay');
        if (overlayMatilampu) {
            overlayMatilampu.classList.replace('active-ui', 'hidden-ui');
            overlayMatilampu.classList.remove('senter-active');
        }
        
        const btnSenter = document.getElementById('btn-toggle-senter');
        if (btnSenter) {
            btnSenter.classList.add('hidden-ui');
            btnSenter.classList.remove('senter-on');
        }
    }
    
    cekTriggerMonster(idLayarTujuan);
...
function handleMenuAction(action) {
    ...
    if (action === "start") {
        ...
        const btnContinue = document.getElementById('btn-main-continue');
        if (btnContinue) btnContinue.classList.add('hidden-ui');
        mulaiLoading(); 
    }
...
['brightness', 'volume', 'music'].forEach(id => {
    ...
    if (slider && output) {
        ...
        slider.addEventListener('input', function() {
            ...
            if (id === 'volume') {
                const volTarget = this.value / 100;
                ['sfx-jumpscare', 'sfx-monster-breath', 'sfx-footsteps'].forEach(idAudio => {
                    const audioElemen = document.getElementById(idAudio);
                    if (audioElemen) audioElemen.volume = volTarget;
                });
            }
...
function munculkanClue(teks) {
    if (teks.startsWith("HINT:") && statusHint === false) {
        return; 
    }
...
function cobaBukaPintuFinal() {
    ...
    if (punyaSemua) {
        tampilkanToastSistem("Sirkuit daya utama aktif! Gerbang pelarian terbuka.", false);
        bukaLayar('screen-final-door');
    } else {
        const belumAda = KUNCI_DIBUTUHKAN.filter(kunci => !koleksiKunciKhusus.includes(kunci));
        tampilkanToastSistem("Sistem interlock aktif! Pintu keluar terkunci. Butuh: " + belumAda.join(", ").toUpperCase(), true);
    }
}
...
function prosesHack() {
    ...
    setTimeout(() => {
        ...
        inventoryPemain[index] = { 
            id: 'flashdisk-data', 
            src: 'assets/items/flashdisk.png', 
            type: 'item', 
...
function cobaBukaPintu(labName, keyId, targetScreenId, elementId) {
    ...
    if (keyIndex !== -1) {
        ...
        tampilkanToastSistem(`Akses Diterima! Pintu ${labName} terbuka.`, false);
        pintu.onclick = function() { bukaLayar(targetScreenId); };
        if (!pintuTerbuka.includes(elementId)) {
            pintuTerbuka.push(elementId);
        }
    } else {
        tampilkanToastSistem(`Pintu ${labName} terkunci rapat. Butuh kunci khusus.`, true);
    }
    saveGame();
}
...
function saveGame(idLayarManual = null) {
    ...
    if (['screen-home', 'screen-loading', 'screen-settings', 'screen-credits', 'screen-achievement', 'screen-ending', 'screen-gameover'].includes(layarSekarang)) {
        return;
    }
...
function loadGameState() {
    ...
    try {
        ...
        inventoryPemain = (gameState.inventory || []).filter(item => item && typeof item === 'object');
        dokumenTerkumpul = (gameState.dokumen || []).filter(doc => doc && typeof doc === 'string');
        koleksiKunciKhusus = (gameState.kunciKhusus || []).filter(kunci => kunci && typeof kunci === 'string');
        pintuTerbuka = (gameState.pintuTerbuka || []).filter(pintu => pintu && typeof pintu === 'string');

        // === A. PULIHKAN STATUS PINTU ===
        pintuTerbuka.forEach(idPintu => {
            const pintu = document.getElementById(idPintu);
            if (pintu) {
                pintu.classList.remove('icon-locked');
                pintu.classList.add('icon-open');
                if (idPintu === 'door-elektro') pintu.onclick = () => bukaLayar('screen-lab-elektro');
                if (idPintu === 'door-matematika') pintu.onclick = () => bukaLayar('screen-lab-matematika');
                if (idPintu === 'door-physics') pintu.onclick = () => bukaLayar('screen-lab-fisika');
                if (idPintu === 'door-chamber') pintu.onclick = () => bukaLayar('screen-chamber');
            }
        });

        // === B. FIX BUG UI: SEMBUNYIKAN ITEM ===
        const hasItem = (id) => inventoryPemain.some(i => i.id === id);
        const sembunyikanElemen = (idDom) => {
            const el = document.getElementById(idDom);
            if (el) el.style.display = 'none';
        };

        if (hasItem('senter')) sembunyikanElemen('item-senter');
        if (hasItem('flashdisk') || hasItem('flashdisk-data')) sembunyikanElemen('item-flashdisk');
        if (hasItem('kamera')) sembunyikanElemen('item-kamera');
        if (hasItem('kunci-matematika') || pintuTerbuka.includes('door-matematika')) sembunyikanElemen('item-idcard');
        if (hasItem('kunci-fisika') || hasItem('kunci-elektro') || pintuTerbuka.includes('door-elektro') || pintuTerbuka.includes('door-physics')) sembunyikanElemen('item-kunci-lab');
        if (hasItem('kunci-chamber') || pintuTerbuka.includes('door-chamber')) sembunyikanElemen('item-kunci-chamber');

        // === C. FIX BUG UI: SEMBUNYIKAN DOKUMEN ===
        const sembunyikanDoc = (keyword, idDom) => {
            if (dokumenTerkumpul.some(d => d.includes(keyword))) {
                sembunyikanElemen(idDom);
            }
        };
        sembunyikanDoc("Log Lobi", "doc-lobby");
        sembunyikanDoc("Rencana Eksperimen", "doc-eksperimen");
        sembunyikanDoc("Laporan Kegagalan", "doc-gagal");
        sembunyikanDoc("PERINGATAN DARURAT", "doc-peringatan");
        sembunyikanDoc("Sinyal frekuensi", "doc-elek1");
        sembunyikanDoc("Log Matematika", "doc-math1");
        sembunyikanDoc("Log Storage", "doc-storage1");
        sembunyikanDoc("Berkas Subjek", "doc-monster");
        sembunyikanDoc("Laporan Final", "doc-physics-final");
        sembunyikanDoc("Catatan Akhir", "doc-chamber");

        renderInventoryUI();
        renderAchievements();
        
        // === TELEPORTASI KE LAYAR TERAKHIR ===
        const targetLayar = gameState.screen || 'screen-map1';
        bukaLayar(targetLayar);
        ...
        if (koleksiKunciKhusus.includes("kunci fasa")) {
...
// --- FUNGSI BARU: RESET ACHIEVEMENT ---
function resetAchievements() {
    const yakin = confirm("PERINGATAN: Semua piala pencapaian akan dihapus permanen! Anda yakin?");
    if (yakin) {
        localStorage.removeItem('project_emp_achievements');
        // Reset status di array memori saat ini
        dataAchievement.forEach(item => item.isUnlocked = false);
        // Render ulang tampilan agar langsung berubah jadi "Locked"
        renderAchievements(); 
        munculkanClue("Sistem di-reset: Semua data Achievement telah dihapus.");
    }
}
...
// ==========================================
// 10. LOGIKA SURVIVAL HORROR (MONSTER & BLACKOUT)
// ==========================================
...
// --- TAMBAHAN LOGIKA SENTER & RADAR GLOBAL ---
let isSenterOn = false;
let radarSenter;       // DIJADIKAN GLOBAL agar bisa dibunuh paksa
let batasWaktuSenter;  // DIJADIKAN GLOBAL agar bisa dibunuh paksa

function toggleSenter() {
    isSenterOn = !isSenterOn;
    const btn = document.getElementById('btn-toggle-senter');
    const overlay = document.getElementById('blackout-overlay');

    if (isSenterOn) {
        btn.classList.add('senter-on');
        overlay.classList.add('senter-active');
        tampilkanToastSistem("Senter dinyalakan. Hati-hati cahaya menarik perhatian.", false);
    } else {
        btn.classList.remove('senter-on');
        overlay.classList.remove('senter-active');
        tampilkanToastSistem("Senter dimatikan.", false);
    }
}

// Daftar ruangan berbahaya (Lobby dan Final Door TIDAK ADA di sini, sehingga otomatis jadi Safe Zone)
const DANGER_ZONES = [
    'screen-map2', 'screen-map3', 'screen-map4', 
    'screen-lab-elektro', 'screen-lab-matematika', 'screen-map7', 
    'screen-storage', 'screen-lab-fisika', 'screen-chamber', 'screen-quarantine'
];

function cekTriggerMonster(idLayarTujuan) {
    // 1. JIKA PEMAIN LARI KE SAFE ZONE (Lobi, Pintu Terakhir, Ending) -> LANGSUNG HENTIKAN MONSTER!
    if (!DANGER_ZONES.includes(idLayarTujuan)) {
        hentikanMonster();
        return;
    }

    // 2. Jika masih di Danger Zone dan monster SUDAH mengejar, biarkan terus mengejar
    if (monsterActive) return; 
    
    // 3. Jika masuk ruangan baru dan monster belum muncul, lempar dadu
    const peluang = Math.random() * 100;
    if (peluang <= 20) {
        picuKehadiranMonster();
    }
}

function picuKehadiranMonster() {
    monsterActive = true;
    const napas = document.getElementById('sfx-monster-breath');
    const langkah = document.getElementById('sfx-footsteps');
    
    if (napas) napas.play().catch(()=>{});
    if (langkah) langkah.play().catch(()=>{});
    
    tampilkanWarningMonster("SUARA LANGKAH KAKI MENDEKAT... SEMBUNYI SEKARANG!");

    batasWaktuSenter = null; 

    // --- RADAR CAHAYA SENTER (SISTEM FLEKSIBEL "NGAKALIN" MONSTER) ---
    radarSenter = setInterval(() => {
        if (!monsterActive) {
            clearInterval(radarSenter);
            if (batasWaktuSenter) clearTimeout(batasWaktuSenter);
        } 
        else if (isSenterOn) {
            if (!batasWaktuSenter) {
                tampilkanWarningMonster("CAHAYA SENTERMU MENARIK PERHATIANNYA! MATIKAN!");
                
                // Pemain punya 6 detik untuk panik mematikan senter
                batasWaktuSenter = setTimeout(() => {
                    if (isSenterOn) { 
                        clearInterval(radarSenter);
                        clearTimeout(timerKematian);
                        picuJumpscareGameOver();
                    }
                }, 6000); 
            }
            ...
            if (batasWaktuSenter) {
                clearTimeout(batasWaktuSenter); 
                batasWaktuSenter = null; 
            }
        }
    }, 500);

    // PEMAIN PUNYA WAKTU 8 DETIK UNTUK KLIK TOMBOL SEMBUNYI
    timerKematian = setTimeout(() => {
        const uiSembunyi = document.getElementById('popup-hide-ui');
        if (!uiSembunyi.classList.contains('active-ui')) {
            picuJumpscareGameOver();
        } else {
            tampilkanToastSistem("Suara langkah kaki menjauh... Ia pergi untuk saat ini.", false);
            hentikanMonster();
        }
    }, 8000);
}

function tampilkanWarningMonster(pesan) {
    const warningBox = document.createElement('div');
    warningBox.className = 'monster-warning-toast';
    warningBox.innerHTML = `⚠️ ${pesan} ⚠️`;
    document.getElementById('game-container').appendChild(warningBox);

    // Otomatis hilang setelah 4 detik agar tidak menutupi layar terus-menerus
    setTimeout(() => {
...
// --- FUNGSI BARU: TOAST UNTUK PEMBERITAHUAN SISTEM ---
function tampilkanToastSistem(pesan, isBahaya = false) {
    const toastBox = document.createElement('div');
    // Jika bahaya pakai desain merah, jika info biasa pakai desain biru
    toastBox.className = isBahaya ? 'monster-warning-toast' : 'system-toast-info';
    toastBox.innerHTML = isBahaya ? `⚠️ ${pesan} ⚠️` : `[ SISTEM ]<br>${pesan}`;
    document.getElementById('game-container').appendChild(toastBox);
...
function hentikanMonster() {
    monsterActive = false;
    
    // BUNUH SEMUA TIMER ANCAMAN SECARA PAKSA!
    clearTimeout(timerKematian);
    if (radarSenter) clearInterval(radarSenter);
    if (batasWaktuSenter) clearTimeout(batasWaktuSenter);

    const napas = document.getElementById('sfx-monster-breath');
    const langkah = document.getElementById('sfx-footsteps');
    if (napas) { napas.pause(); napas.currentTime = 0; }
    if (langkah) { langkah.pause(); langkah.currentTime = 0; }
}

function picuJumpscareGameOver() {
    hentikanMonster();
    const popup = document.getElementById('popup-jumpscare');
    const suara = document.getElementById('sfx-jumpscare');

    popup.classList.replace('hidden-ui', 'active-ui');
    unlockAchievement("achiev-5"); 
    
    if (suara) { suara.currentTime = 0; suara.play(); }

    setTimeout(() => {
        popup.classList.replace('active-ui', 'hidden-ui');
        localStorage.removeItem('project_emp_save_state'); 
        bukaLayar('screen-gameover'); 
    }, 4000);
}

function jalankanSiklusListrik() {
    if (intervalListrik) clearInterval(intervalListrik);
    intervalListrik = setInterval(() => {
        const layarAktif = document.querySelector('.screen.active');
        if (layarAktif && DANGER_ZONES.includes(layarAktif.id) && !isBlackout) {
            isBlackout = true;
            document.getElementById('blackout-overlay').classList.replace('hidden-ui', 'active-ui');
            
            if (inventoryPemain.some(item => item.id === 'senter')) {
                document.getElementById('btn-toggle-senter').classList.remove('hidden-ui');
            }

            // Memakai toast bahaya (merah)
            tampilkanToastSistem("Sirkuit Daya Utama Putus! Segera perbaiki di Lab Elektronika!", true);
            
            setTimeout(() => { picuKehadiranMonster(); }, 3000); 
        }
    }, Math.floor(Math.random() * (15000 - 10000 + 1)) + 10000); 
}

function pulihkanDaya() {
    isBlackout = false;
    isSenterOn = false; 
    hentikanMonster();
    document.getElementById('blackout-overlay').classList.replace('active-ui', 'hidden-ui');
    document.getElementById('blackout-overlay').classList.remove('senter-active');
    ...
    tampilkanToastSistem("Daya berhasil dipulihkan. Suasana kembali terang.", false);
    jalankanSiklusListrik(); 
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadAchievementsOnly(); 
    renderAchievements(); 
    
    // Logika memunculkan tombol Continue secara otomatis jika ada save data
    const adaSave = localStorage.getItem('project_emp_save_state');
    const btnContinue = document.getElementById('btn-main-continue');
    
    if (adaSave && btnContinue) {
        btnContinue.classList.remove('hidden-ui');
    }
});