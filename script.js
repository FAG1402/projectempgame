...
// Daftar ruangan tempat monster bisa muncul (Lobi 'screen-map1' DIHAPUS agar jadi safe zone)
const DANGER_ZONES = [
...
function picuKehadiranMonster() {
    ...
    let batasWaktuSenter = null;

    let radarSenter = setInterval(() => {
        // Jika monster sudah pergi (pemain berhasil sembunyi), matikan radar
        if (!monsterActive) {
            clearInterval(radarSenter);
            if (batasWaktuSenter) clearTimeout(batasWaktuSenter);
        } 
        // Jika monster masih ada dan pemain menyalakan senter
        else if (isSenterOn) {
            // Mulai hitung mundur kematian JIKA belum dimulai
            if (!batasWaktuSenter) {
                munculkanClue("CAHAYA SENTERMU MENARIK PERHATIANNYA! MATIKAN SEKARANG!!");
                
                // Pemain punya 3,5 detik untuk panik mematikan senter
                batasWaktuSenter = setTimeout(() => {
                    // Jika setelah 3,5 detik senter MASIH nyala = GAME OVER
                    if (isSenterOn) { 
                        clearInterval(radarSenter);
                        clearTimeout(timerKematian);
                        picuJumpscareGameOver();
                    }
                }, 6000); // <--- UBAH ANGKA INI JIKA INGIN LEBIH LAMA (Misal: 4000 = 4 Detik)
            }
        } 
        // Jika pemain mematikan senter SEBELUM batas waktu habis
        else {
            if (batasWaktuSenter) {
                clearTimeout(batasWaktuSenter); // Batalkan hukuman mati
                batasWaktuSenter = null; // Reset radar agar pemain bisa nyalakan/matikan lagi
            }
        }
    }, 500);
...
function jalankanSiklusListrik() {
    ...
    intervalListrik = setInterval(() => {
        ...
        if (layarAktif && DANGER_ZONES.includes(layarAktif.id) && !isBlackout) {
            ...
            // TAMPILKAN TOMBOL SENTER JIKA PEMAIN SUDAH MEMBUNGUTNYA
