...
function loadGameState() {
    ...
    if (savedState) {
        ...
        // ==========================================
        // TAMBAHAN: RESTART SIKLUS HORROR JIKA SUDAH PUNYA KUNCI FASA
        // ==========================================
...
function toggleSenter() {
    ...
    if (isSenterOn) {
        ...
        munculkanClue("Senter dinyalakan. Jarak pandang membaik, tapi cahayanya berisiko.");
    } else {
        btn.classList.remove('senter-on');
        overlay.classList.remove('senter-active');
        munculkanClue("Senter dimatikan. Kegelapan kembali menyelimuti.");
    }
}
...
function picuKehadiranMonster() {
    ...
    munculkanClue("Suara langkah kaki dan napas berat terdengar... Sembunyi, SEKARANG!");
    ...
    radarSenter = setInterval(() => {
        if (!monsterActive) {
            ...
            if (!batasWaktuSenter) {
                munculkanClue("CAHAYA SENTERMU MENARIK PERHATIANNYA! MATIKAN SEKARANG!!");
    ...
    timerKematian = setTimeout(() => {
        ...
        if (!uiSembunyi.classList.contains('active-ui')) {
            ...
            munculkanClue("Suara langkah kaki menjauh... Ia pergi untuk saat ini.");
            hentikanMonster();
        }
    }, 8000); 
}
...
function jalankanSiklusListrik() {
    ...
    // Timer 10-15 detik untuk keperluan testing (Bisa diganti 90000 nanti)
    intervalListrik = setInterval(() => {
        ...
        if (layarAktif && DANGER_ZONES.includes(layarAktif.id) && !isBlackout) {
            ...
            munculkanClue("PERINGATAN: Sirkuit Daya Utama Putus! Segera ke Lab Elektronika untuk menyambungkan ulang kabel panel!");
...
function pulihkanDaya() {
    ...
    munculkanClue("Daya berhasil dipulihkan. Suasana kembali terang.");
    jalankanSiklusListrik(); 
}
